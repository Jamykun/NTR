...
public class Graph_Temps_MknMoyen {
    ...
    private static final String GRAPH_NAME = "Graph_Temps_MknMoyen";
    
    public static void add(int temps, int mkn, DistancePointAcces dist) {
        if(dist == DistancePointAcces.LOIN) {
            if(!pointsUltiLoin.containsKey(temps)) {
                pointsUltiLoin.put(temps, new ArrayList<>());
            }
            pointsUltiLoin.get(temps).add(mkn);
        } else {
            if(!pointsUltiProche.containsKey(temps)) {
                pointsUltiProche.put(temps, new ArrayList<>());
            }
            pointsUltiProche.get(temps).add(mkn);
        }
    }
    ...
    public static void GenerateGraph() throws IOException {
    	...
    	for(Integer key : keysLoin) {
            ...
            bw.write(key+";"+(sum/(double)pointsUltiLoin.get(key).size()));
            bw.newLine();
    	}
    	...
    	for(Integer key : keysProche) {
            ...
            bw2.write(key+";"+(sum/(double)pointsUltiProche.get(key).size()));
            bw2.newLine();
    	}
